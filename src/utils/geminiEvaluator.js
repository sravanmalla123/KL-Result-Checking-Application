...
async function evaluateSimulation(filename, scenario, numImages, text = '') {
  ...
  // Scan text for explicit references to AI image generation or household photos
  const textLower = (text || '').toLowerCase();
  const hasAiTool = ["midjourney", "dall-e", "stable-diffusion", "stable diffusion"].some(k => textLower.includes(k));
  const hasAiImageTerm = ["ai-generated image", "ai generated image", "image generated by ai", "synthetic image", "ai diagram", "ai-generated diagram", "ai generated diagram"].some(k => textLower.includes(k));
